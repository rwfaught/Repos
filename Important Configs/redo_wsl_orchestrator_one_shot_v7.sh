#!/usr/bin/env bash
# redo_wsl_orchestrator_one_shot_v7.sh
#
# REDO WSL ORCHESTRATOR — one-shot known-good bootstrap for fresh Ubuntu WSL.
#
# Intended first run:
#   bash redo_wsl_orchestrator_one_shot_v7.sh --yes
#
# Optional repo restore:
#   bash redo_wsl_orchestrator_one_shot_v7.sh --yes --archive /mnt/c/Users/accou/Downloads/projects.tar.gz
#
# Design:
...
OPENCLAW_PRIMARY="ollama/qwen3.5:9b"
OPENCLAW_FALLBACKS='["ollama/qwen3.5:4b","ollama/qwen3.5:2b","ollama/qwen3:0.6b"]'
...
usage() {
  cat <<'EOF'
Usage:
  bash redo_wsl_orchestrator_one_shot_v7.sh [options]
...
Options:
  ...
  --include-heavy-models        Pull/test 30B/32B models. Off by default.
  --include-risky-models        Pull/test qwen3.6:35b. Off by default and not recommended.
  ...
  --primary MODEL               OpenClaw primary model. Default: ollama/qwen3.5:9b.
  --help                        Show this help.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --include-heavy-models) INCLUDE_HEAVY_MODELS=1; shift ;;
    --include-risky-models) INCLUDE_RISKY_MODELS=1; INCLUDE_HEAVY_MODELS=1; shift ;;
...
log "Priming sudo credentials once near the start"
cat <<'EOF'
You may be asked for your Linux password now.
This prevents the script from failing much later after long model downloads.
EOF
sudo -v
...
EOF
  ...
  if [[ "$SKIP_MODELS" != "1" ]]; then
    if [[ "$MODEL_PACK" == "minimal" ]]; then
      ...
      # Default core is intentionally safe/stable. Heavy 30B/32B models are opt-in because
      # qwen3-coder:30b has crashed the terminal on this laptop, and qwen3.6:35b has hard-locked it.
      MODELS=(qwen3:0.6b qwen3.5:2b qwen3.5:4b qwen3.5:9b)
      if [[ "$INCLUDE_HEAVY_MODELS" == "1" ]]; then
        MODELS+=(qwen3-coder:30b qwen2.5-coder:32b qwen3:32b)
      fi
...
EOF
  ...
  verify_model_gpu() {
    ...
    timeout 90s ollama run "$model" "Reply with OK only." > "$out_file" 2>&1 &
    local run_pid=$!
  ...
  run_model_verification_suite() {
    ...
    log "Creating low-context aliases before verification"
    if [[ "$INCLUDE_HEAVY_MODELS" == "1" ]]; then
      ...
      log "Skipping heavy 30B/32B aliases. Use --include-heavy-models only if intentionally stress-testing."
    fi
    ...
    local verify_models=(qwen3:0.6b qwen3.5:2b qwen3.5:4b qwen3.5:9b)

    if [[ ("$VERIFY_TIER" == "medium" || "$VERIFY_TIER" == "all") && "$INCLUDE_HEAVY_MODELS" == "1" ]]; then
      verify_models+=(qwen3-coder:30b-4k qwen2.5-coder:32b-4k qwen3:32b-4k)
    elif [[ "$VERIFY_TIER" == "medium" || "$VERIFY_TIER" == "all" ]]; then
      log "Heavy verification tier requested, but --include-heavy-models was not set. Staying on safe models."
    fi

    if [[ "$VERIFY_TIER" == "all" && "$INCLUDE_RISKY_MODELS" == "1" ]]; then
      verify_models+=(qwen3.6:35b-4k)
    fi
...
log "Writing summary"
{
  ...
  echo "- Heavy 30B/32B models are intentionally excluded unless --include-heavy-models is used."
