...
| Dossier/case mapping and task-readiness seams | WATCH | Source/test/docs seams and docs-only reviews support the neutral abstraction. Phase 4 persistence integration is completed and CTO-ratified: explicit case-scoped source-material and extracted-fact identities now survive local save/load, legacy anonymous entries remain compatible, and ordinary whole-packet updates cannot bypass protected-entry transition semantics. Commits `27fafd1` and `ff13eb6` are durable on `main`. Provenance, evidence links, and workflow completion remain open. The next likely useful-work boundary is the narrow evidence-linked synthesis transformation seam, subject to separate authorization. The ratified Small-Business AI Implementation Dossier may exercise general capabilities, but consulting-specific concerns remain outside the core by default. This does not prove runtime/provider/model behavior, a product wedge, concurrency, or production readiness. |
...
| Revenue-bearing proving use case / neutral core | ACTIVE | Roger's Small-Business AI Implementation Dossier remains the ratified proving use case. One fictional end-to-end manual dry run is complete and CTO/coordinator-ratified for durable evidence recording: it demonstrates one manually executable, reviewable fictional dossier engagement only—not repeatability, real-client usefulness or willingness to pay, implementation behavior, provider/model competence, product-wedge suitability, or production readiness. The controlled dossier-workflow roadmap phase remains incomplete. The highest-ranked missing capability is the narrow evidence-linked synthesis transformation seam: convert a bounded source packet and classified evidence registers into a reviewable recommendation package while preserving contradictions, missing information, qualifications, decision gates, and non-proofs. Phase 4 persistence integration is completed and CTO-ratified; provenance, evidence links, and workflow completion remain open. Founder Cockpit static work remains closed; no final wedge is selected; Phase 387 remains parked. | `EVIDENCE_LINKED_SYNTHESIS_TRANSFORMATION_SEAM`; product-wedge selection or Phase 387 resume only if separately authorized. | `TRACKS_AND_OPEN_THREADS.md` |
