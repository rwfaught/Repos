...
| `COORDINATION_DOC_UPDATE_OBLIGATION_CLOSURE_RULE` | OPEN_DEFERRED_AFTER_RECONCILIATION_CLOSURE | Worker/Codex reports must state `Coordination-doc update needed: YES / NO / UNSURE`. The unresolved governance question is whether every `YES` or `UNSURE` must receive an explicit CTO/coordinator disposition and remain open until accepted and durably updated, deferred with a recorded reason, rejected as unnecessary, or sent for further review. | `COORDINATION_DOC_UPDATE_OBLIGATION_CLOSURE_RULE_REVIEW` | `TRACKS_AND_OPEN_THREADS.md` |
