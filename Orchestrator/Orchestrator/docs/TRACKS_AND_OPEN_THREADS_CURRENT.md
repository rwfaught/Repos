...
| Founder Cockpit sidecar | LIVE_READ_ONLY_OUTCOME_RANKED | The static prototype and representative-context transfer are accepted. Live implementation remains unimplemented and unproven; the next ranked outcome is `LIVE_READ_ONLY_FOUNDER_COCKPIT_MINIMUM_USEFUL_SURFACE`, a read-only window over existing authority rather than a second control system. |
...
| Founder Cockpit live read-only surface | RANKED_NOT_IMPLEMENTED | Static information model, prototype, and representative-context transfer are accepted; live surface remains unimplemented and unproven. | `LIVE_READ_ONLY_FOUNDER_COCKPIT_MINIMUM_USEFUL_SURFACE` under a separate bounded implementation authorization. | `docs/GOVERNED_RESEARCH_V1_CLOSEOUT_AUTONOMY_AND_FOUNDER_VISIBILITY_RECONCILIATION_DECISION.md` |
