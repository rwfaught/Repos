...
- `RELAY_POWERSHELL_5_1_GIT_BATCH_PROFILE_V1_STARTUP_DOCTRINE_UPDATE` is a
  docs-only doctrine patch pending CTO review. It records five observed
  routine Windows PowerShell 5.1 Git command-construction failures and adds a
  mandatory simplification profile; no runtime or product behavior changed.
...
| Founder Cockpit sidecar | ACTIVE_ADVISORY_SIDECAR_PLATFORM_BLOCKED | Information architecture is accepted for a tool-neutral fixture specification. Codex Sites is deferred because its governing privacy and deployment contract is unverified; no implementation or live integration is authorized. Next likely boundary: `FOUNDER_COCKPIT_TOOL_NEUTRAL_FIXTURE_INFORMATION_MODEL_AND_WIREFRAME_SPEC`. |
...
| `COORDINATION_DOC_UPDATE_OBLIGATION_CLOSURE_RULE` | ACCEPTED_PATCH_PENDING_COMMIT_PUSH | CTO disposition: `ACCEPT_AND_UPDATE`. The mandatory CTO/coordinator disposition and closure rule has been added to doctrine. This documentation-patch stage resolves only after CTO review; final thread closure still requires commit/push durability verification. | `COORDINATION_DOC_UPDATE_OBLIGATION_CLOSURE_RULE_COMMIT_PUSH_VERIFY` | `TRACKS_AND_OPEN_THREADS.md` |
| `RELAY_POWERSHELL_5_1_GIT_BATCH_PROFILE_V1_STARTUP_DOCTRINE_UPDATE` | PENDING_CTO_REVIEW | Docs-only mandatory routine Git command-construction profile records five observed command-design failures; no runtime or product behavior changed, Founder Cockpit semantics are unchanged, and no product wedge is selected. | `RELAY_POWERSHELL_5_1_GIT_BATCH_PROFILE_V1_CTO_REVIEW_THEN_COMMIT_PUSH_VERIFY` | `TRACKS_AND_OPEN_THREADS.md` |
