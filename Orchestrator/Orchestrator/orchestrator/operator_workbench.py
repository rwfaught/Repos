...
class Workbench:
    ...
    def _presentation(item: dict[str, Any], evidence: dict[str, Any], elapsed: float) -> dict[str, Any]:
        ...
        review_context = {"requested_outcome": proposal.get("understanding", item.get("packet", {}).get("workbench_objective", "")), "declared_checks": proposal.get("checks", item.get("packet", {}).get("success_criteria", [])), "limitations": ["These checks confirm only the declared file and output. They do not establish broader semantic correctness or production readiness."]}
        if item.get("state") == "executing":
            return {"headline": "Working on the declared task", "keep": "Not ready yet", "explanation": "The worker is still running. No decision can be recorded yet.", "can_decide": False, "elapsed_seconds": elapsed, **review_context}
        if item.get("state") == "accepted":
            return {"headline": "Accepted and recorded", "keep": "Accepted by the operator", "explanation": "The acceptance record was saved with the reason you provided.", "can_decide": False, "elapsed_seconds": elapsed, **review_context}
        if can_decide:
            return {"headline": "Declared checks passed", "keep": "Ready for your decision", "explanation": "The declared file changed and the recorded checks passed. You may accept, reject, or request a correction.", "can_decide": True, "recommended_action": "Review the outcome and choose the decision that reflects your judgment.", "elapsed_seconds": elapsed, "changed_paths": changed, **review_context}
        ...
        return {"headline": "Needs correction before a decision", "keep": "Do not accept yet", "explanation": explanation, "can_decide": False, "can_revise": True, "recommended_action": "Describe what should change, then review and explicitly authorize a revised task.", "elapsed_seconds": elapsed, "changed_paths": changed, "technical_status": task_status or error or "unknown", **review_context}
