"""Pure handoff packet ready-state readback data."""
...
PHASE = 365
NAME = "product_task_packet_handoff_packet_ready_state_readback"
BOUNDARY = "PHASE365_PRODUCT_TASK_PACKET_HANDOFF_PACKET_READY_STATE_READBACK_SOURCE_TEST_DOCS"
MARKER = "PHASE365_PRODUCT_TASK_PACKET_HANDOFF_PACKET_READY_STATE_READBACK_SOURCE_TEST_DOCS_PROVEN=PASS"
...
ACCEPTED_FACTS = (
    "Phase 364 selected a ready-state readback as the next bounded move",
    "Phase 364 did not execute the selected next boundary",
...
READINESS_RECOMMENDATION_AND_INFERENCE = (
    "packet is ready_for_next_bounded_move_readback_only when all readiness gates are satisfied",
    "readiness is a recommendation/status field, not execution authority",
    "future execution requires separate explicit authority and validation",
)
...
BLOCKING_CONDITIONS = (
    ...
    "missing selected next boundary",
...
REQUIRED_EVIDENCE_BEFORE_FUTURE_EXECUTION = (
    "new explicit execution-authority boundary",
    "fresh working tree and ref verification",
    "allowed-file or operation scope",
    "validation plan for the future boundary",
    "operator/coordinator authorization when required",
    "non-proof caveats preserved",
)

READY_STATE_RECOMMENDATION = {
    "status": "ready_for_next_bounded_move_readback_only",
    "recommended_next_boundary": RECOMMENDED_NEXT_BOUNDARY,
    "execution_authority": False,
    "reason": "readiness gates are represented as deterministic source data only",
}

INVALID_READY_STATE_CLAIMS = (
    "ready-state readback means handoff execution occurred",
    "ready-state readback means handoff packet execution occurred",
    "ready-state readback means worker dispatch occurred",
    "ready-state readback means patch application occurred",
    "ready-state readback means route selection execution occurred",
    "ready-state readback means provider/model execution occurred",
    "ready-state readback means next-boundary execution occurred",
    "ready-state recommendation means production readiness",
    ...
    "Phase 365 supersedes Phase 335 capsule proof",
)
...
FUTURE_PHASE_ASSERTION_DOCTRINE = (
    ...
    "future Phase 366 files may exist after a later legitimate boundary implements them",
)
...
LOCKOUT_TEXT = (
    ...
    "No oz",
    "No push",
    "No Phase 366 implementation",
)


def read_product_task_packet_handoff_packet_ready_state_readback() -> dict[str, Any]:
    """Return deterministic handoff packet ready-state readback data."""
    return {
        "phase": PHASE,
        "name": NAME,
        "boundary": BOUNDARY,
        ...
        "ready_state_purpose": (
            "Record whether the handoff packet is ready for the next bounded move; "
            "ready-state readback is deterministic and non-executing."
        ),
        "accepted_facts": list(ACCEPTED_FACTS),
        "readiness_recommendation_and_inference": list(READINESS_RECOMMENDATION_AND_INFERENCE),
        ...
        "ready_state_recommendation": dict(READY_STATE_RECOMMENDATION),
        "invalid_ready_state_claims": list(INVALID_READY_STATE_CLAIMS),
