...
def _review_artifact_payload(
    ...
    local_first_policy_input = structured_input if structured_input is not None else {}
    return {
        "phase": "PHASE_257",
        ...
        "general_answer_local_first_policy": build_general_answer_local_first_fallback_policy(
            local_first_policy_input
        ),
    }
...
def _run_general_answer_input(path_text: str, artifact_path: str = "") -> ManualReviewCliResult:
    ...
    if artifact_path:
        write_error = _write_review_artifact(
            artifact_path,
            _review_artifact_payload(review, result, artifact_path, value),
        )
        if write_error:
            return _result(
                exit_code=2,
