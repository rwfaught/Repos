...
def main(argv: list[str] | None = None) -> int:
    ...
    while index < len(args):
        if args[index] == "--objective" and index + 1 < len(args) and args[index + 1].strip():
            ...
            print(json.dumps({"accepted": False, "blocked": True, "detail": "Usage: python -m orchestrator.coordinator_agent_loop_cli --objective <text> [--format json|markdown|operator]"}, indent=2))
            return 2
