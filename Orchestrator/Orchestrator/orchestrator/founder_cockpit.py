...
<section class='card'><h2>Bounded task execution</h2><p>The Cockpit remains read-only. For one explicitly authorized bounded coding task, open the separate local Operator Workbench.</p><p><a href='http://127.0.0.1:8766/workbench'>Open Operator Workbench V1</a> <small>(start its launcher first)</small></p></section><section class='card'><h2>Authority details</h2><p>Each displayed claim retains its source identity. This view cannot resolve disagreement or make authority decisions.</p><table><thead><tr><th>Source</th><th>Path</th><th>Availability</th></tr></thead><tbody>{source_rows}</tbody></table></section></main></body></html>"""
