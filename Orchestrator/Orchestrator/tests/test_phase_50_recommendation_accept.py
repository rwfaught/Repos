...
class Phase50RecommendationAcceptTests(unittest.TestCase):
    ...
    def test_a_accept_existing_accept_result_updates_only_target_record(self):
        run = create_run("phase50 accept existing")
        reviewer_target = self._task_id("task_phase50_accept_target")
        reviewer_other = self._task_id("task_phase50_accept_other")
        ...
        self._persist_recommendation(reviewer_target, "accept_result", "Accept this result")
        self._persist_recommendation(reviewer_other, "accept_result", "Unrelated record")
        ...
        self.assertIn("Recommendation record accepted.", text)
        ...
        self.assertTrue(target_record_after.get("accepted"))
        self.assertTrue(str(target_record_after.get("accepted_at", "")).strip())
    ...
    def test_b_wrong_recommendation_type_rejected_without_mutation(self):
        run = create_run("phase50 wrong type")
        reviewer_task_id = self._task_id("task_phase50_wrong_type")
        self._create_reviewer_emitter_task(run["id"], reviewer_task_id, "source_wrong", "artifact_wrong")
        self._persist_recommendation(reviewer_task_id, "manual_followup", "Not accept_result")
        ...
        with patch.object(
            sys,
            "argv",
            ["main.py", "recommendation-accept", "--reviewer-task", reviewer_task_id, "--run", run["id"]],
        ):
            output = io.StringIO()
            with redirect_stdout(output):
                main.accept_recommendation()

        self.assertIn("Only accept_result recommendations can be accepted", output.getvalue())
    ...
    def test_c_missing_recommendation_record_not_found_no_mutation(self):
        run = create_run("phase50 missing recommendation")
        missing_reviewer_task_id = self._task_id("task_phase50_missing")
        ...
        with patch.object(
            sys,
            "argv",
            ["main.py", "recommendation-accept", "--reviewer-task", missing_reviewer_task_id, "--run", run["id"]],
        ):
            output = io.StringIO()
            with redirect_stdout(output):
                main.accept_recommendation()
    ...
    def test_d_wrong_run_scope_stops_cleanly_without_mutation(self):
        run_a = create_run("phase50 run a")
        reviewer_task_id = self._task_id("task_phase50_wrong_scope")
        self._create_reviewer_emitter_task(run_a["id"], reviewer_task_id, "source_a", "artifact_a")
        self._persist_recommendation(reviewer_task_id, "accept_result", "Only in run A")

        run_b = create_run("phase50 run b")
        ...
        with patch.object(
            sys,
            "argv",
            ["main.py", "recommendation-accept", "--reviewer-task", reviewer_task_id, "--run", run_b["id"]],
        ):
            output = io.StringIO()
            with redirect_stdout(output):
                main.accept_recommendation()
    ...
    def test_e_no_hidden_behavior_change_no_task_or_queue_mutation(self):
        run = create_run("phase50 no hidden behavior")
        reviewer_task_id = self._task_id("task_phase50_no_hidden")
        self._create_reviewer_emitter_task(run["id"], reviewer_task_id, "source_hidden", "artifact_hidden")
        self._persist_recommendation(reviewer_task_id, "accept_result", "Accept target")
        ...
        with patch.object(sys, "argv", ["main.py", "recommendation-accept", "--reviewer-task", reviewer_task_id]):
            output = io.StringIO()
            with redirect_stdout(output):
                main.accept_recommendation()

        self.assertIn("Recommendation record accepted.", output.getvalue())
    ...
    def test_f_accepted_record_is_inspectable(self):
        run = create_run("phase50 inspectable")
        reviewer_task_id = self._task_id("task_phase50_inspectable")
        self._create_reviewer_emitter_task(run["id"], reviewer_task_id, "source_inspect", "artifact_inspect")
        self._persist_recommendation(reviewer_task_id, "accept_result", "Inspectable acceptance")
        ...
        self.assertTrue(updated.get("accepted"))
        self.assertTrue(str(updated.get("accepted_at", "")).strip())

    def test_g_no_active_run_message_and_no_mutation(self):
        state = load_state()
        ...
        with patch.object(
            sys,
            "argv",
            ["main.py", "recommendation-accept", "--reviewer-task", self._task_id("task_phase50_no_active")],
        ):
            output = io.StringIO()
            with redirect_stdout(output):
                main.accept_recommendation()
