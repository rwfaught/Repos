...
from orchestrator.product_task_packet_provider_policy_contract_readback import (
    BOUNDARY,
    MARKER,
    read_product_task_packet_provider_policy_contract_readback,
)


MARKER_TEXT = "PHASE359_PRODUCT_TASK_PACKET_PROVIDER_POLICY_CONTRACT_READBACK_SOURCE_TEST_DOCS_PROVEN=PASS"
NEXT_BOUNDARY = "PHASE360_PRODUCT_TASK_PACKET_DOMAIN_GENERAL_INTAKE_CONTRACT_READBACK_SOURCE_TEST_DOCS"


class Phase359ProviderPolicyContractReadbackTests(unittest.TestCase):
    def setUp(self):
        self.readback = read_product_task_packet_provider_policy_contract_readback()

    def test_deterministic_identity(self):
        self.assertEqual(self.readback, read_product_task_packet_provider_policy_contract_readback())
        self.assertEqual(self.readback["phase"], 359)
    ...
    def test_source_basis_and_spine(self):
        for phase in ("phase_349", "phase_351", "phase_352", "phase_354", "phase_355", "phase_356", "phase_357", "phase_358"):
            self.assertIn(phase, self.readback["source_basis"])
        self.assertIn("worker_dispatch_contract_readback_phase_358", self.readback["completed_packet_spine"])

    def test_phase_specific_contracts_exist(self):
        self.assertTrue(self.readback["provider_policy_gates"])
        self.assertTrue(self.readback["model_provider_claim_limits"])
        self.assertIn("provider/model execution blocked", self.readback["provider_policy_gates"])
        self.assertIn("no live provider/model availability claim", self.readback["provider_policy_gates"])

    def test_invalid_claims_and_stop_conditions(self):
        self.assertIn("provider policy means provider/model execution occurred", self.readback["invalid_claims"])
        for stop in (
