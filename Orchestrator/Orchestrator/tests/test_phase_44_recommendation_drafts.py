...
class Phase44RecommendationDraftsTests(unittest.TestCase):
    ...
    def test_a_shows_drafts_for_active_run_and_accept_no_draft_needed(self):
        run = create_run("phase44 active run drafts")
        task_accept = self._task_id("task_phase44_accept")
        task_followup = self._task_id("task_phase44_followup")
        task_repair = self._task_id("task_phase44_repair")
        ...
        self._persist_recommendation(task_accept, "accept_result", "Acceptable output.")
        self._persist_recommendation(task_followup, "manual_followup", "Needs manual follow-up.")
        self._persist_recommendation(task_repair, "repair_candidate", "Needs repair.")

        counts_before = self._snapshot_counts()
        with patch.object(sys, "argv", ["main.py", "recommendation-drafts"]):
            output = io.StringIO()
        ...
        self.assertIn(f"Recommendation drafts for run: {run['id']}", text)
        ...
        self.assertIn("draft_needed=no", text)
        self.assertIn("No draft task needed for accept_result.", text)
        self.assertIn(f"reviewer_task_id={task_followup}", text)
        self.assertIn("proposed_role=reviewer", text)
        self.assertIn("proposed_title=Follow-up review for", text)
        self.assertIn(f"reviewer_task_id={task_repair}", text)
        self.assertIn("proposed_role=coder", text)
        self.assertIn("proposed_title=Repair for", text)
        self.assertEqual(counts_after, counts_before)

    def test_b_explicit_run_scopes_drafts(self):
        run_a = create_run("phase44 explicit run A")
        task_a = self._task_id("task_phase44_a")
        self._create_reviewer_emitter_task(run_a["id"], task_a)
        self._persist_recommendation(task_a, "manual_followup", "A only.")

        run_b = create_run("phase44 explicit run B")
        task_b = self._task_id("task_phase44_b")
        self._create_reviewer_emitter_task(run_b["id"], task_b)
        self._persist_recommendation(task_b, "repair_candidate", "B only.")

        with patch.object(sys, "argv", ["main.py", "recommendation-drafts", "--run", run_a["id"]]):
            output = io.StringIO()
            with redirect_stdout(output):
                main.print_recommendation_drafts()

        text = output.getvalue()
        self.assertIn(f"Recommendation drafts for run: {run_a['id']}", text)
    ...
    def test_c_no_matching_recommendations_message_and_no_mutation(self):
        run = create_run("phase44 no matching")
        counts_before = self._snapshot_counts()

        with patch.object(sys, "argv", ["main.py", "recommendation-drafts", "--run", run["id"]]):
            output = io.StringIO()
            with redirect_stdout(output):
                main.print_recommendation_drafts()

        self.assertIn(f"Recommendation drafts for run: {run['id']}", output.getvalue())
